...
public class MainActivity extends AppCompatActivity {
    ...
    class JSBridge {
        @JavascriptInterface
        public void runUssd(String code, int simSlot) { runOnUiThread(() -> executeUSSD(code, simSlot)); }

        @JavascriptInterface
        public void sendSMS(String number, String message, int simSlot) { runOnUiThread(() -> sendSMSInternal(number,message,simSlot)); }

        @JavascriptInterface
        public void setSystemColor(String color) { runOnUiThread(() -> changeSystemColor(color)); }
    }
    ...
    private void executeUSSD(String code,int simSlot){
        if(Build.VERSION.SDK_INT<26){ sendUSSDResult("USSD requires Android 8+"); return; }

        if(ContextCompat.checkSelfPermission(this, Manifest.permission.CALL_PHONE)!=PackageManager.PERMISSION_GRANTED
           || ContextCompat.checkSelfPermission(this, Manifest.permission.READ_PHONE_STATE)!=PackageManager.PERMISSION_GRANTED){
            pendingUSSD=code+"|"+simSlot;
            ActivityCompat.requestPermissions(this,new String[]{Manifest.permission.CALL_PHONE,Manifest.permission.READ_PHONE_STATE},REQUEST_USSD_PERMISSION);
            return;
        }
        ...
        try{
            ...
            int subId=-1;
            for(SubscriptionInfo info:list){ if(info.getSimSlotIndex()==simSlot){ subId=info.getSubscriptionId(); break; } }
            ...
            tm.sendUssdRequest(code,new TelephonyManager.UssdResponseCallback(){
                @Override
                public void onReceiveUssdResponse(TelephonyManager telephonyManager,String request,CharSequence response){ sendUSSDResult(response.toString()); }
                @Override
                public void onReceiveUssdResponseFailed(TelephonyManager telephonyManager,String request,int failureCode){ sendUSSDResult("USSD failed: "+failureCode); }
            }, new Handler(Looper.getMainLooper()));

        }catch(Exception e){ sendUSSDResult("USSD error: "+e.getMessage()); }
    }

    private void sendUSSDResult(String message){ final String safe = message.replace("\\","\\\\").replace("'","\\'").replace("\n","\\n"); webView.post(() -> webView.evaluateJavascript("showResult('"+safe+"')",null)); }
    ...
    private void sendSMSInternal(String number,String message,int simSlot){
        if(ContextCompat.checkSelfPermission(this,Manifest.permission.SEND_SMS)!=PackageManager.PERMISSION_GRANTED
           || ContextCompat.checkSelfPermission(this,Manifest.permission.READ_PHONE_STATE)!=PackageManager.PERMISSION_GRANTED){
            ...
            ActivityCompat.requestPermissions(this,new String[]{Manifest.permission.SEND_SMS,Manifest.permission.READ_PHONE_STATE},REQUEST_SMS_PERMISSION);
            return;
        }
        ...
        try{
            ...
            if(Build.VERSION.SDK_INT>=22){ smsManager=SmsManager.getSmsManagerForSubscriptionId(subId); } else { smsManager=SmsManager.getDefault(); }
    ...
    private void sendResult(String msg){ final String safe=msg.replace("\\","\\\\").replace("'","\\'").replace("\n","\\n"); webView.post(() -> webView.evaluateJavascript("showResult('"+safe+"')",null)); }
    ...
    private class SmsReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context,Intent intent){
            ...
            for(Object pdu:pdus){
                SmsMessage sms;
                if(Build.VERSION.SDK_INT>=23){ String format=bundle.getString("format"); sms=SmsMessage.createFromPdu((byte[])pdu,format); }
    ...
    private void startForegroundService(){
        if(Build.VERSION.SDK_INT>=26){
            String channelId="foreground_sms_service";
            NotificationChannel channel=new NotificationChannel(channelId,"SMS & USSD Service",NotificationManager.IMPORTANCE_LOW);
            NotificationManager nm=(NotificationManager)getSystemService(NOTIFICATION_SERVICE);
            nm.createNotificationChannel(channel);

            NotificationCompat.Builder builder=new NotificationCompat.Builder(this,channelId)
                    .setContentTitle("SMS & USSD Dashboard")
                    .setContentText("Running in foreground for live messages")
