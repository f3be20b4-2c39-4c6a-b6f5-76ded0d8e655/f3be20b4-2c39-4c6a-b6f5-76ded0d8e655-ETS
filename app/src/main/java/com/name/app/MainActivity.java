...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // Load existing inbox and sent messages after a short delay
    ...
    private void executeUSSD(String code, int simSlot) {
        ...
        try {
            ...
            if(subId==-1) subId=list.get(0).getSubscriptionId(); // fallback
    ...
    private void loadAllSMS(){
        ...
        // Inbox
        Cursor cursorInbox = getContentResolver().query(Uri.parse("content://sms/inbox"),null,null,null,"date DESC");
        if(cursorInbox!=null){
            int numIdx = cursorInbox.getColumnIndex("address");
            int bodyIdx = cursorInbox.getColumnIndex("body");
            while(cursorInbox.moveToNext()){
                String num = cursorInbox.getString(numIdx);
                String msg = cursorInbox.getString(bodyIdx);
        ...
        // Sent
        Cursor cursorSent = getContentResolver().query(Uri.parse("content://sms/sent"),null,null,null,"date DESC");
        if(cursorSent!=null){
            int numIdx = cursorSent.getColumnIndex("address");
            int bodyIdx = cursorSent.getColumnIndex("body");
            while(cursorSent.moveToNext()){
                String num = cursorSent.getString(numIdx);
                String msg = cursorSent.getString(bodyIdx);
                final String number = num.replace("'","\\'");
                final String message = msg.replace("'","\\'");
    ...
    private void startForegroundService(){
        if(Build.VERSION.SDK_INT>=26){
            ...
            NotificationChannel channel=new NotificationChannel(channelId,"SMS Service",NotificationManager.IMPORTANCE_LOW);
